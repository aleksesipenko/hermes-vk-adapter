...
Реакции на жизненный цикл выключены по умолчанию. `VK_REACTION_FAILED_ID`
ставится только при неудачной отправке самим адаптером: Hermes не даёт
платформенному адаптеру хука «агент завершился с ошибкой».
...
устаревшее сообщение. VK идентифицирует реакцию
...
VK_REACTION_FAILED_ID="5"       # ставится, если отправка ответа не удалась
```
