...
### 3. Создать токен сообщества

1. Перейди в `Управление` -> `Настройки` -> `Работа с API` -> `Ключи доступа`.
2. Нажми `Создать ключ`.
3. Выдай минимально нужные права:
   - сообщения сообщества/messages;
   - документы/docs, чтобы Hermes мог отправлять сгенерированные файлы;
   - фото/photos, если VK UI требует это для вложений или будущей отправки фото.
4. Не выдавай права личного аккаунта и не используй personal/user token.
5. Скопируй токен один раз и храни его как секрет.
