...
4. Выдай минимально нужные права доступа:
   ...
   - `docs` / документы сообщества: нужно для отправки сгенерированных файлов
     через `docs.getMessagesUploadServer` и `docs.save`.
5. `photos` / фотографии сообщества сейчас не обязателен для текущего адаптера:
   входящие фото приходят через Long Poll как вложения, а исходящие файлы
   отправляются через VK Docs. Выдавай `photos` только если планируешь будущую
   нативную отправку фото или VK UI требует это для твоего сценария.
